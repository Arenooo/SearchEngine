...
ParsedData ResultSetToParsedData(sql::ResultSet* set)
{
...
std::string Search(const std::string& query)
{
    ...
    while(titleSet->next())
    {
        ...
        if(urls.insert(url).second)
            ret += to_json(ResultSetToParsedData(titleSet)) + ',';
    }
    while(descriptionSet->next())
    {
        ...
        if(urls.insert(url).second)
            ret += to_json(ResultSetToParsedData(descriptionSet)) + ',';
    }
    while(textSet->next())
    {   
        found = true;
        ...
        if(urls.insert(url).second)
            ret += to_json(ResultSetToParsedData(textSet)) + ',';
    }
